...
class MazeBase(object):
    ...
    def setPortalPairs(self, nodes):
        # add the portal to the to the nodes in question to the lookup table
        for pair in list(self.portalPairs.values()):
            nodes.setPortalPair(*pair)

    def connectHomeNodes(self, nodes):
        key = nodes.createHomeNodes(*self.homeoffset) # give the homeoffset set in the Maze in question
...
class Maze1(MazeBase):
    # inherit from MazeBase. 
    def __init__(self):
        MazeBase.__init__(self)
        self.name = "maze1"
        self.portalPairs = {0:((0, 17), (27, 17))} # for this one we only have one portal that connects two nodes
        self.homeoffset = (11.5, 14)
        self.homenodeconnectLeft = (12, 14)
        self.homenodeconnectRight = (15, 14)
        self.pacmanStart = (15, 26) # set the starting point for pacman as default
